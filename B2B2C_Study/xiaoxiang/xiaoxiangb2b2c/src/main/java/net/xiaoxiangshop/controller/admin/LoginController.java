...
public class LoginController extends BaseController {
	...
	public ResponseEntity<?> getSmsCode(String username, String password) {
        ...
        if(!admin.getEncodedPassword().equals(pwd)){
			return responseReslut(null,null,100,"密码错误");
		}
		smsService.sendSmsCode("1",admin.getMobile());
		return responseReslut(null,null,200,"短信验证码已发送");
	}
	/**
	 * 校验短信验证码
	 * @param username
	...
	public ResponseEntity<?> checkSmsCode(String username, String password,String smsCode) {
		...
		ResponseEntity<?> reslut=smsService.checkSmsCode("1",admin.getMobile(),smsCode);
